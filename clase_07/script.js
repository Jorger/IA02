...
function inicio()
{
	...
	var conMinMax = 1; //Índica que usará el algoritmo de Minimax...
	var txtFichas = ["X", "O"];
	var fichaJugador = 1;
	var pc = 0;

	//Mostrará el número de ciclos y/o proceso que realiza Minimax...
	var tmpNumerodeCiclos = 0;
	//true: Mostrará los pasos realizados por Minimax...
	...
	*/
	...
	function movimientoPC()
	{
		...
		else
		{
			...
			{
				fila = 1;
				columna = posTablero - 4;
			}
			else
			{
				fila = 2;
				columna = posTablero - 7;
			}
		}
		var fichaPC = 1;
		if(fichaJugador == 1)
	...
	function Min()
	{
		...
		if(revisarTriqui(2)) return 1; //EL PC hizo Triqui..
		if(entablas()) return 0; //Emptados...
		var aux, mejor = 9999;
		for (var i=0;i<9;i++)//2, 4
		{
			if (vectorMinMAX[i] == 0)
			{
